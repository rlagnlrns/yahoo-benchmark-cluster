...
object KafkaRedisAdvertisingStream {
  ...
  def writeRedisTopLevel(campaign_window_counts_Iterator: Iterator[org.apache.spark.sql.Row], redisHost: String) {
    val pool = new Pool(new JedisPool(new JedisPoolConfig(), redisHost, 6379, 2000))
  ...
  private def writeWindow(pool: Pool, campaign_window_counts: org.apache.spark.sql.Row) : String = {
